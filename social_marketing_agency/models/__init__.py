# Copyright (C) 2026 Vertel Sverige AB.
# License AGPL-3.0 or later (https://www.gnu.org/licenses/agpl).

from . import social_brand_focus_mixin
from . import social_brand
from . import social_brand_credential
from . import social_brand_color
from . import social_brand_font
from . import social_brand_logo
from . import social_brand_asset
from . import res_partner
from . import res_users
from . import social_agency_document_type
from . import social_agency_document
from . import communication_policy
from . import communication_plan
from . import social_marketing_post_template
from . import social_marketing_post
from . import social_content_source
from . import social_marketing_listening_topic
from . import social_marketing_stream
from . import social_marketing_account
from . import social_marketing_live_post
from . import social_marketing_competitor
from . import social_marketing_media_asset
from . import social_marketing_message
from . import dashboard_dashboard
from . import social_agency_invite
