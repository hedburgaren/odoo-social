...
from odoo.addons.social_marketing.controllers.main import SocialController
...
class SocialLinkedinController(SocialController):
    ...
    @http.route('/social_marketing_linkedin/callback', type='http', auth='user')
    def social_marketing_linkedin_callback(self, access_token=None, code=None, state=None, **kw):
        """
        We can receive
        ...
        if not request.env.user.has_group('social_marketing.group_social_marketing_manager'):
        ...
        if kw.get('error') not in ('user_cancelled_authorize', 'user_cancelled_login'):
            ...
            media = request.env.ref('social_marketing_linkedin.social_media_linkedin')
            ...
            try:
                ...
                request.env['social_marketing.account']._create_linkedin_accounts(access_token, media)
        ...
        return request.redirect('/web?#%s' % url_encode({
            'action': request.env.ref('social_marketing.action_social_marketing_stream_post').id,
            'view_type': 'kanban',
            'model': 'social_marketing.stream.post',
        }))
    ...
    @http.route('/social_marketing_linkedin/comment', type='http', auth='user', methods=['POST'])
    def social_marketing_linkedin_add_comment(self, stream_post_id, message=None, comment_id=None, **kwargs):
        stream_post = self._get_social_marketing_stream_post(stream_post_id, 'linkedin')
        return json.dumps(stream_post._linkedin_comment_add(message, comment_id))

    @http.route('/social_marketing_linkedin/delete_comment', type='json', auth='user')
    def social_marketing_linkedin_delete_comment(self, stream_post_id, comment_id, **kwargs):
        stream_post = self._get_social_marketing_stream_post(stream_post_id, 'linkedin')
        return stream_post._linkedin_comment_delete(comment_id)

    @http.route('/social_marketing_linkedin/get_comments', type='json', auth='user')
    def social_marketing_linkedin_get_comments(self, stream_post_id, comment_urn=None, offset=0, comments_count=20):
        stream_post = self._get_social_marketing_stream_post(stream_post_id, 'linkedin')
    ...
    def _linkedin_get_access_token(self, linkedin_authorization_code, media):
        ...
        linkedin_app_id = request.env['ir.config_parameter'].sudo().get_param('social_marketing.linkedin_app_id')
        linkedin_client_secret = request.env['ir.config_parameter'].sudo().get_param(
