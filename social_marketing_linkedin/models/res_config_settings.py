...
class ResConfigSettings(models.TransientModel):
    ...
    linkedin_use_own_account = fields.Boolean("Use your own LinkedIn Account",
                                              config_parameter='social_marketing.linkedin_use_own_account',
                                              help="""Check this if you want to use your personal LinkedIn Developer 
                                              Account instead of the provided one.""")
    linkedin_app_id = fields.Char("App ID", config_parameter='social_marketing.linkedin_app_id',
                                  compute='_compute_linkedin_app_id', inverse='_inverse_linkedin_app_id')
    linkedin_client_secret = fields.Char("App Secret", config_parameter='social_marketing.linkedin_client_secret',
                                         compute='_compute_linkedin_client_secret',
                                         inverse='_inverse_linkedin_client_secret')
    ...
    def _compute_linkedin_app_id(self):
        user_is_manager = self.env.user.has_group('social_marketing.group_social_marketing_manager')
        ...
        for setting in self:
            if user_is_manager:
                self.env['ir.config_parameter'].sudo().set_param(
                    'social_marketing.linkedin_app_id', setting.linkedin_app_id)

    @api.depends('linkedin_use_own_account')
    def _compute_linkedin_client_secret(self):
        user_is_manager = self.env.user.has_group('social_marketing.group_social_marketing_manager')
        linkedin_client_secret = self.env['ir.config_parameter'].sudo().get_param('social_marketing.linkedin_client_secret')
        for setting in self:
