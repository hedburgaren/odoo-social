...
class SocialStream(models.Model):
    """"social.streams are used to fill the 'Feed' view that allows users to follow the social_marketing.media activity
    based on their interest (a Facebook Page, a Twitter hashtag, ...).

    They use the social_marketing.media third party API to fetch the stream data and create social.stream.posts
    that are displayed on the Feed kanban view. """

    _name = 'social_marketing.stream'
    ...
    def refresh_all(self):
        ...
        new_content = False
        for stream in self.env['social_marketing.stream'].search([]):
    ...
    def _fetch_stream_data(self):
        ...
        This is the method responsible for creating the social_marketing.stream.posts using the social_marketing.media
        third party API.

        It will be called manually every time we need to refresh the social_marketing.stream data:
            - social_marketing.stream creation/edition
            - 'Feed' kanban loading
            - 'Refresh' button on 'Feed' kanban
            - ...

        This method should return 'True' if new social_marketing.posts are inserted,  please check the 'refresh_all' method for
